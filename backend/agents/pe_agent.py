...
class PEAgent:
    ...
    def _tax_reply(self, extracted: dict, message: str) -> str:
        lines = ["关于PE税负，以典型场景（假设€200万年利润）为例：\n"]
        lines.append("**PE构成前后税负对比：**")
        lines.append("- PE前：仅5%股息预提税 ≈ €70,000/年")
        lines.append("- PE后：KSt 15%+SolZ+GewSt≈14%+预提税 ≈ €596,000/年")
        lines.append("- **每年多缴约 €526,000 + HGB合规成本**\n")
        lines.append("实际金额取决于您的德国利润规模。请在「税负量化」页面输入具体数字查看精确计算。")
        return "\n".join(lines)
