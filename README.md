...
| **法律文档语义检索** | BM25 关键词检索（离线、无向量库依赖） | 内置中德协定 / AO / BEPS / HGB 关键词索引（可补充原文至 legal_docs/ 继续扩充） |
...
| **税负暴露量化** | KSt 15% + SolZ + GewSt ≈30% + 中国境外税收抵免 + HGB 合规成本 = 年度 € 金额（PE 前德国无征税权） |
...
│   ├── rag/
│   │   └── document_store.py       # BM25 关键词检索（ChromaDB 未启用）
│   ├── knowledge_graph/
...
| 过程文档与演示表现 | 25% | 10 页方案 PPT + 16 次 AI 协作日志 + 117 个自动化测试 |
