...
基于 **规则引擎 + 全文检索 + 知识图谱** 的税务合规平台（可选接入大模型增强问答），专为中国企业出海德国场景设计的常设机构（PE）风险分析工具。
...
- Secrets 配置：`DEEPSEEK_API_KEY`（可选，启用大模型增强问答；未配置时自动使用规则引擎模式）
- 德国法原文：10 部德国法律已抽取为 JSON 置于 `legal_docs/extracted_law_pe/`（17,433 分块、约 3,750 万字符），本地离线 BM25 全文检索；云端因体积可裁剪后部署
...
| 功能 | 说明 |
|------|------|
| **自由文本输入** | 粘贴企业德国业务描述，关键词规则自动识别 15 个法律要件，秒级出结果 |
...
| 功能 | 技术实现 | 说明 |
|------|---------|------|
| **税务智能问答** | 意图规则匹配 + 内置知识库（可选大模型增强） | 自然语言 PE 问答，置信度评分，建议后续操作 |
| **法律知识图谱** | NetworkX 构建，层级交互展示 | 34 个节点 / 68 条边，五层合规推理模型 |
| **法律文档检索** | BM25 关键词检索（离线、无向量库依赖） | 10 部德国法全文索引（17,433 分块，可继续扩充） |
...
| **税负暴露量化** | KSt 15% + SolZ + GewSt ≈29.8% + 中国境外税收抵免 + HGB 合规成本 = 年度 € 金额（PE 前德国无征税权） |
...
| 技术实现与用户体验 | 25% | Streamlit 单架构 9 页面，规则引擎 + 全文检索 + 知识图谱 |
| 过程文档与演示表现 | 25% | 10 页方案 PPT + 16 次 AI 协作日志 + 105 个自动化测试 |
...
# 105 passed — 规则引擎、税负计算、雷达图、HGB清单、检索文档、问答、反馈收集
```
